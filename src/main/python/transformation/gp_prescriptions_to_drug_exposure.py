...
def gp_prescriptions_to_drug_exposure(wrapper: Wrapper) -> List[Wrapper.cdm.DrugExposure]:
    ...
    dmd_mapper = wrapper.code_mapper.generate_code_mapping_dictionary(
        'dm+d',
        restrict_to_codes=list(filter(None, set(source['dmd_code']))))
    # Read v2 drug codes not available in Athena
    # read2_mapper = wrapper.code_mapper.generate_code_mapping_dictionary(
    #     'Read',
    #     restrict_to_codes=list(filter(None, set(source['read_2']))))
    ...
    for _, row in source.iterrows():
        ...
        if row['dmd_code']:
            ...
            if mapping:
                ...
                drug_col = 'dmd_code'
        # TODO: implement mappings for other drug columns
