...
from ..util import add_dot_to_icdx_code, create_hes_visit_occurrence_id, create_hes_visit_detail_id, refactor_icdx_code
...
def hesin_diag_to_condition_occurrence(wrapper: Wrapper) -> List[Wrapper.cdm.ConditionOccurrence]:
    ...
    df['diag_icd9_dot'] = df['diag_icd9_dot'].apply(add_dot_to_icdx_code)
    df['diag_icd10_dot'] = df['diag_icd10_dot'].apply(add_dot_to_icdx_code)
