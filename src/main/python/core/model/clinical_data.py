...
class ConditionOccurrence(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    condition_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    ...
    condition_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    stop_reason = Column(String(20))
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
    condition_source_value = Column(String(50))
    condition_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    condition_status_source_value = Column(String(50))
    condition_status_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class DeviceExposure(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    device_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    ...
    device_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    unique_device_id = Column(String(50))
    quantity = Column(Integer)
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
    device_source_value = Column(String(100))
    device_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class DrugExposure(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    drug_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    ...
    drug_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    stop_reason = Column(String(20))
    refills = Column(Integer)
    quantity = Column(Numeric)
    days_supply = Column(Integer)
    sig = Column(Text)
    route_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    lot_number = Column(String(50))
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
    drug_source_value = Column(String(50))
    drug_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class FactRelationship(base):
    ...
    domain_concept_id_1 = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    domain_concept_id_2 = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    fact_id_1 = Column(Integer, nullable=False)
    fact_id_2 = Column(Integer, nullable=False)
    relationship_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
...
class Measurement(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    measurement_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    measurement_date = Column(Date, nullable=False)
    measurement_datetime = Column(DateTime)
    measurement_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    operator_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    value_as_number = Column(Numeric)
    value_as_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    unit_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    range_low = Column(Numeric)
    range_high = Column(Numeric)
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
    measurement_source_value = Column(String(50))
    measurement_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class Note(base):
    ...
    note_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    note_class_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    note_title = Column(String(250))
    note_text = Column(Text, nullable=False)
    encoding_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    language_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
...
class NoteNlp(base):
    ...
    note_id = Column(ForeignKey('cdm_cllear.note.note_id'), nullable=False, index=False)
    section_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    snippet = Column(String(250))
    offset = Column(String(250))
    lexical_variant = Column(String(250), nullable=False)
    note_nlp_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), index=False)
...
class Observation(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    observation_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    observation_date = Column(Date, nullable=False)
    observation_datetime = Column(DateTime)
    observation_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    value_as_number = Column(Numeric)
    value_as_string = Column(String(60))
    value_as_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    qualifier_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    unit_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
    observation_source_value = Column(String(50))
    observation_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    unit_source_value = Column(String(50))
...
class ObservationPeriod(base):
    ...
    period_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
...
class Person(base):
    ...
    gender_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    year_of_birth = Column(Integer, nullable=False)
    month_of_birth = Column(Integer)
    day_of_birth = Column(Integer)
    birth_datetime = Column(DateTime)
    race_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    ethnicity_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    ...
    race_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    ethnicity_source_value = Column(String(50))
    ethnicity_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class Death(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), primary_key=True, index=False)
    death_date = Column(Date, nullable=True)
    death_datetime = Column(DateTime)
    death_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    cause_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    cause_source_value = Column(String(50))
    cause_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class ProcedureOccurrence(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    procedure_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    procedure_date = Column(Date, nullable=False)
    procedure_datetime = Column(DateTime, nullable=False)
    procedure_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    modifier_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    quantity = Column(Integer)
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    visit_occurrence_id = Column(ForeignKey('cdm_cllear.visit_occurrence.visit_occurrence_id'), index=False)
    procedure_source_value = Column(String(50))
    procedure_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    qualifier_source_value = Column(String(50))
...
class Specimen(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    specimen_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    specimen_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    specimen_date = Column(Date, nullable=False)
    specimen_datetime = Column(DateTime)
    quantity = Column(Numeric)
    unit_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    anatomic_site_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    disease_status_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
...
class VisitOccurrence(base):
    ...
    person_id = Column(ForeignKey('cdm_cllear.person.person_id'), nullable=False, index=False)
    visit_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False, index=False)
    ...
    visit_type_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'), nullable=False)
    provider_id = Column(ForeignKey('cdm_cllear.provider.provider_id'))
    care_site_id = Column(ForeignKey('cdm_cllear.care_site.care_site_id'))
    visit_source_value = Column(String(50))
    visit_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    admitting_source_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
    admitting_source_value = Column(String(50))
    discharge_to_concept_id = Column(ForeignKey('omopcdm.concept.concept_id'))
