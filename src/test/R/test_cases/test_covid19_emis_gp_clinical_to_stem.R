...
declareTest(2602, 'Value is -9000001')
add_baseline(eid=2602)
add_covid19_emis_gp_clinical(eid = 2602, event_dt = '24/02/2020', code = 163020007, value = '-9000001')
...
add_covid19_emis_gp_clinical(eid = 2610, code_type = 2, code='128053003', value = '-9999999')
expect_condition_occurrence(person_id = 2610, condition_concept_id = 4133004)

declareTest(2611, 'Covid19 EMIS GP clinical to stem, EMISATTACHMENT and code_type 5')
add_baseline(eid = 2611)
add_covid19_emis_gp_clinical(eid = 2611, code_type = 5, code='EMISATTACHMENT', value = '-9000099')
expect_observation(person_id = 2611, observation_source_value = 'EMISATTACHMENT')
