...
hesin['epistart'] = min_d + pd.to_timedelta(np.random.randint(d, size=n_hes), unit='d')
hesin['epiend'] = min_d + pd.to_timedelta(np.random.randint(d, size=n_hes), unit='d')
hesin['spell_index'] = np.random.choice(range(1, 967), size=n_hes)
hesin['elecdate'] = pd.to_datetime('18/02/2010')
hesin['admidate'] = min_d + pd.to_timedelta(np.random.randint(d, size=n_hes), unit='d')
hesin['admimeth'] = np.random.choice(range(1, 101), size=n_hes)
hesin['admisorc'] = np.random.choice(range(1, 101), size=n_hes)
hesin['disdate'] = min_d + pd.to_timedelta(np.random.randint(d, size=n_hes), unit='d')
...
hesin_diag['diag_icd9'] = np.random.choice(['V252', '6262', '78909', '4549', '7890',
                                            '7865', '6359', '4556', '1749', '5509',
                                            '5742', '6117', '6289'], size=n_diag)
hesin_diag['diag_icd10'] = np.random.choice(['I10', 'E119', 'Z864', 'E780', 'J459',
                                             'Z511', 'Z867', 'Z921', 'E039', 'I209',
                                             'I48', 'I259', 'C509'], size=n_diag)
...
hesin_oper['opdate'] = min_d + pd.to_timedelta(np.random.randint(d, size=n_oper), unit='d')
